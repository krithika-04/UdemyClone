_677‍.x([["sendOtpVerification",()=>sendOtpVerification]]);const db= require('../helper/db')
const user= db.user;
  ...
  exports.login=async (req,res)=>{
     ...
     exports.currentInstructor = async(req,res)=>{
        try {
            ...
            const instructor= await Instructor.findOne({where:{user_id:req.body.user_id
            }}).then((result)=>{
                console.log(result)
                if(result!=null)
                res.json({ok:true})
     ...
     exports.verifyOtp= async(req,res)=>{
        try {
            ...
            throw new Error("Otp details are empty")
                ...
                const verifiDetails= await EmailVerification.findOne({where:{user_id:user_id}})
                if(verifiDetails.length<=0)
            {//no records
                throw new Error("Your account details doesn't or have been verified already.Please sign up or login") 
            }
            else{
                const {expiresAt,hashedotp}=verifiDetails;
